...
For a fixed mean of 1 UMI/cell we want to test different effect sizes. We assess different effect sizes by varying the applied factor from: 1,1.1,1.2,...,2. We will do this and test across different individuals per group: 1, 5, 10, 15, 20,..., 35. Since we are testing for 1000 cells/individual we can pool all cells for individuals. For example, 35 individuals/group would be 35,000 cells in the null variance group and 35,000 cells in the effect size group for a total of 70 individuals.
...
We use the following equation to estimate variance by assuming a multiple hypergeometric model for the data. [Kim et al.](https://www.cell.com/cell/fulltext/S0092-8674%2824%2901144-9) uses method of moments to estimate the variance.

$$
\hat{\sigma}_{g,HG} \approx \frac{1}{n_{cells}}\sum_c \frac{Y^2_{cg} - Y_{cg}(1-q)}{N^2_cq^2 - N_c q(1-q)} - \left( \frac{1}{n_{cells}}\sum_c\frac{Y_{cg}}{N_cq} \right)
$$
Where for $c$ cells and $g$ genes, $Y_{cg}$ is the observed transcript count, $N_c$ is the library size, $n_{cells}$ is the number of cells, and $q$ is the sampling efficiency.
